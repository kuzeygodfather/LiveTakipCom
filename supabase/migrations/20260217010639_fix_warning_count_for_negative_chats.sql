/*
  # Fix Warning Count to Include Negative Sentiment Chats

  1. Changes
    - Update recalculate_personnel_stats function to count warnings for chats with:
      * overall_score < 50 OR
      * sentiment = 'negative'
    - This ensures all negative chats trigger warnings, not just low-scoring ones
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      CASE 
        ...
        ELSE 50
      END as avg_score,
      COUNT(*) FILTER (WHERE ca.overall_score < 50 OR ca.sentiment = 'negative') as warning_count
    FROM chats c
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE 50
    END,
    COUNT(*) FILTER (WHERE ca.overall_score < 50 OR ca.sentiment = 'negative')
  FROM chats c
  LEFT JOIN chat_analysis ca ON ca.chat_id = c.id
  WHERE c.agent_name IS NOT NULL 
    AND c.agent_name != ''
  ...
  SELECT 
    ...
    COUNT(*) FILTER (WHERE ca.overall_score < 50 OR ca.sentiment = 'negative') as total_issues,
    COALESCE(AVG(
      CASE 
